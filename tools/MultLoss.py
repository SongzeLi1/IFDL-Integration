...
class WeightedBCE(nn.Module):
    def __init__(self, weights=[0.2, 0.8]):  # W_pos=0.2,W_neg=0.8, weights=[W_pos, W_neg]
...
class WeightedDiceLoss(nn.Module):
    def __init__(self, weights=[0.5, 0.5]): # W_neg=0.2,W_pos=0.8, weights=[W_neg, W_pos]
...
class WeightedDiceBCE(nn.Module):
    def __init__(self,dice_weight=1,BCE_weight=1,lovasz_weight=0):
        super(WeightedDiceBCE, self).__init__()
        self.BCE_loss = WeightedBCE(weights=[0.8, 0.2])
        self.dice_loss = WeightedDiceLoss(weights=[0.2, 0.8])   # orig: weights=[0.5, 0.5]
    ...
    def forward(self, inputs, targets):
        ...
        dice_BCE_loss = self.dice_weight * dice + self.BCE_weight * BCE + lovasz * self.lovasz_weight   # orig: dice_BCE_loss = self.dice_weight * dice + self.BCE_weight * BCE
